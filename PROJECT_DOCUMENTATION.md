...
## 9. Diretrizes para evolucao
- Manter padrao `View + ViewController + FlowDelegate` ao criar novas scenes.
- Evitar acoplamento de API diretamente em Views.
- Concentrar regras de request e parse em `Service`/`Client`.
- Preservar fluxo de navegacao centralizado no `HiringAppFlowController`.
- Garantir que recursos de debug (RocketSim) fiquem isolados por flag de compilacao.
